...
class Dataset():
  ...
  def extract_random_segments_for_given_patient(self, patient_no, num_segs_chosen_per_patient):   #helper
    ...
    np.save(f"/content/drive/My Drive/Cross-spectrum-EEG/datasets/stats/stats2.npy", self.stats)
    ...
    random.shuffle(selected_tuples)
    while num_segs_chosen_per_patient<len(selected_tuples):
      _ = selected_tuples.pop()
  ...
  def create(self, num_segs_chosen_per_patient):      #main
    ...
    for p in range(self.num_patients): 
      if (p)%20==0 or p+1==self.num_patients:
        print(f"patient_no: {p+1}: {len(self.data_list)}")
...
np.save(f"/content/data_set2.npy", data_set.data_list)
